...
def plot_cities_2(sf, title, cities, color):
    ...
    city_id = []
    for i in cities:
        city_id.append(df[df.DIST_NAME == i.upper()].index.get_values()[0])
    plot_map_fill_multiples_ids(title, city_id, sf,x_lim=None,y_lim=None,figsize=(11, 9),color=color)

#plotting by city name
south = ['kanniyakumari', 'erode', 'namakkal']
plot_cities_2(sf, 'DIST', south, 'c')
plt.show()
